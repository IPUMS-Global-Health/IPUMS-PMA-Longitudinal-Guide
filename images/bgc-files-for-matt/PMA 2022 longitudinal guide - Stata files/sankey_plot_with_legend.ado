...
*! v1.32_DAR by Dale Rhoda - move legend(off) before `options' in line 573 so the user can show a legend if s/he wishes to
...
program sankey_plot_with_legend
...
syntax varlist [if],  [width0(varname) width1(varname) sharp(real 7) ///
	...
	frame `newframe': {
		...
		*display `"`rrbarr'"'
		two `toplot' `rrbarr' `totext'  , legend(off) `options' ylabel("") 
