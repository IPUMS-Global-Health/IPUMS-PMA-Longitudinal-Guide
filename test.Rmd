...
Chapter 4 demonstrated how to calculate key family planning indicators for PMA panel data, and how to visualize the results in grouped bar charts. In Chapter 5, we demonstrate two additional data visualization tools: **heatmaps** built with `r funlink(ggplot2)` and **alluvial plots** built with `r funlink(ggalluvial)`, an extension package for `r funlink(ggplot2)`. 
