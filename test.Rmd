...
We've constructed a single diagram showing all six samples available from IPUMS PMA, and we'll go over each level in turn.
...
hh <- hh %>% 
  mutate(
    ...
    pop = case_when(
      !is.na(GEOCD) ~ paste0(COUNTRY, "\n", GEOCD),
      !is.na(GEONG) ~ paste0(COUNTRY, "\n", GEONG),
      TRUE ~ COUNTRY
    )
  )
